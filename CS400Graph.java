...
public class CS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) throws NoSuchElementException {
    	...
    	// initializing frontier, first path, starting vertex and ending vertex
    	PriorityQueue<Path> queue = new PriorityQueue<Path>(); 
    	Path initalPath = new Path(new Vertex(start));
    	Vertex currVariable = this.vertices.get(start);
    	Vertex endVertex = this.vertices.get(end);
    	// Hashtable holds settled vertices and their shortest distance
    	Hashtable<Vertex, Double> distances = new Hashtable<Vertex, Double>(); 
    	// adding paths to queue(frontier)
    	for (Edge edge: currVariable.edgesLeaving) {
    		queue.add(new Path(initalPath, edge));
    	}
    	Path shortestPath = initalPath; 
    	// marking start as visited
    	distances.put(currVariable, initalPath.distance); 
    	// iterate until queue is empty
    	while (!queue.isEmpty() && currVariable != endVertex) { 
    		Path currentPath = queue.peek(); 
    		currVariable = queue.poll().end; 
    		// update queue(frontier) if current isn't the ending vertex
    		if (currVariable != endVertex) { 
    			for (Edge edge: currVariable.edgesLeaving) {
    				queue.add(new Path(currentPath, edge));
    			}
    		}
    		shortestPath = currentPath;
    		distances.put(currVariable, shortestPath.distance);
    	}		
    	// throw exception if ending vertex wasn't visited
